class ImageConstant {
  static String imgEllipse2Stroke = 'assets/images/img_ellipse_2_stroke.svg';

  static String imgVector = 'assets/images/img_vector.png';

  static String imgArrowleft = 'assets/images/img_arrowleft.svg';

  static String imgQatarplinkocu = 'assets/images/img_qatarplinkocu.svg';

  static String imgEllipse36 = 'assets/images/img_ellipse3_6.png';

  static String imgEllipse33 = 'assets/images/img_ellipse3_3.png';

  static String imgEllipse3 = 'assets/images/img_ellipse3.png';

  static String imgFolder = 'assets/images/img_folder.svg';

  static String imgFire = 'assets/images/img_fire.svg';

  static String imgEllipse34 = 'assets/images/img_ellipse3_4.png';

  static String imgEllipse32 = 'assets/images/img_ellipse3_2.png';

  static String imgBg = 'assets/images/img_bg.svg';

  static String imgEllipse360x60 = 'assets/images/img_ellipse3_60x60.png';

  static String imgEllipse35 = 'assets/images/img_ellipse3_5.png';

  static String imgVectorIndigo500 = 'assets/images/img_vector_indigo_500.png';

  static String imgEllipse31 = 'assets/images/img_ellipse3_1.png';

  static String imageNotFound = 'assets/images/image_not_found.png';

  static String qatarImage = 'assets/images/img_ellipse3.png';
}
